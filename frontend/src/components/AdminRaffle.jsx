            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-zinc-50 dark:bg-zinc-950/50 text-zinc-500 text-xs uppercase font-semibold">
                  ...
                  <th className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800">Acción (Marcar)</th>
                </tr>
              </thead>
              <tbody className="text-sm">
                {buyers.length === 0 ? <tr><td colSpan="4" className="px-6 py-12 text-center text-zinc-500">Sin operaciones recientes.</td></tr> : buyers.map(b => (
                  <tr key={b.ticket_number} className="border-b border-zinc-100 dark:border-zinc-800/50">
                    ...
                    <td className="px-6 py-4">
                      <button onClick={() => toggleStatus(b.ticket_number, b.status)} className="px-4 py-2 bg-zinc-900 hover:bg-zinc-800 text-white dark:bg-white dark:hover:bg-zinc-200 dark:text-zinc-900 font-medium rounded-lg text-xs transition-all shadow-sm">
                        {b.status === 'reserved' ? 'Marcar Pagado' : 'Marcar No Pagado'}
                      </button>
                    </td>
                  </tr>
                ))}
