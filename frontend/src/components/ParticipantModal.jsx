...
export default function ParticipantModal({ raffleId, ticket, pad, pricePerTicket, onClose, onUpdated, onDelete, showAlert }) {
  ...
  const handleSellerChange = async (value) => {
    ...
    try {
      const res = await fetch('/api/admin_update_ticket_seller.php', {
        ...
        credentials: 'include',
        body: JSON.stringify({ raffle_id: raffleId, ticket_code: localTicket.ticket_code, seller_id: sellerId }),
      });
  ...
  const handleStatusChange = async (newStatus) => {
    ...
    try {
      const res = await fetch('/api/admin_bulk_update_status.php', {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ raffle_id: raffleId, ticket_numbers: localTicket.ticket_numbers, new_status: newStatus }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      const res = await fetch('/api/admin_update_ticket_notes.php', {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ raffle_id: raffleId, ticket_code: localTicket.ticket_code, admin_notes: notes }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handleDelete = async () => {
    setDeleting(true);
    const deleted = await onDelete(localTicket);
    setDeleting(false);
    if (deleted) onClose();
  };
      ...
      <div className="relative min-h-full flex items-center justify-center p-4">
        <div className="relative bg-white dark:bg-zinc-900 p-8 rounded-3xl max-w-sm w-full shadow-2xl border border-zinc-200 dark:border-zinc-800 my-8">
          ...
          <div className="flex gap-2">
            ...
            >
              <Trash2 size={15} /> {localTicket.ticket_numbers.length > 1 ? 'Liberar' : 'Eliminar'}
            </button>
          </div>
        </div>
      </div>
