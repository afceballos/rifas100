...
export default function VerifyParticipationModal({ raffle, pad, onClose }) {
  ...
  const handleSearch = async (e) => {
    ...
    setResult(null);
    try {
      const res = await fetch(`/api/verify_participation.php?raffle_id=${raffle.id}&ticket_number=${encodeURIComponent(query)}`);
      const data = await res.json();
      if (data.success) setResult(data.found ? data : null);
    } catch {
      setResult(null);
    }
    setSearched(true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-zinc-900 p-8 rounded-3xl max-w-md w-full shadow-2xl border border-zinc-200 dark:border-zinc-800 max-h-[90vh] overflow-y-auto">
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6">Busca tu ticket para comprobar el estado de tu participación.</p>

        <form onSubmit={handleSearch} className="space-y-3">
          <input
            type="number" min="0" required
            placeholder={pad ? '0'.repeat(pad) : 'Número de boleto'}
            ...
            {result ? (
              <>
                <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-3">
                  Se encontraron resultados para <span className="font-bold text-zinc-900 dark:text-white">{query}</span>
                </p>
                <div className="p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-100 dark:border-zinc-800 space-y-2 text-sm">
