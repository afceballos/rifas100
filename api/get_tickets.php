...
try {
    $stmt = $pdo->prepare("SELECT id, slug, title, description, background_image, payment_info, online_payment_link, organizer_name, organizer_photo, organizer_phone, organizer_email, theme_color, number_style, bg_color, allow_seller_selection, price_per_ticket, draw_date, total_tickets, number_start, is_published FROM raffles WHERE slug = ?");
    $stmt->execute([$slug]);
    $raffle = $stmt->fetch();
