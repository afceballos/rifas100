...
if (empty($data['title']) || empty($data['price_per_ticket']) || empty($data['draw_date'])) {
    echo json_encode(['success' => false, 'error' => 'Faltan datos requeridos']);
    exit;
}

$numberMode = ($data['number_mode'] ?? 'digits') === 'range' ? 'range' : 'digits';

if ($numberMode === 'range') {
    // Rango personalizado: el admin elige desde y hasta qué número van los boletos.
    $rangeStart = isset($data['range_start']) ? (int)$data['range_start'] : -1;
    $rangeEnd = isset($data['range_end']) ? (int)$data['range_end'] : -1;

    if ($rangeStart < 0 || $rangeEnd <= $rangeStart) {
        echo json_encode(['success' => false, 'error' => 'El rango de números no es válido']);
        exit;
    }

    $number_start = $rangeStart;
    $total_tickets = $rangeEnd - $rangeStart + 1;

    if ($total_tickets > 10000) {
        echo json_encode(['success' => false, 'error' => 'El rango no puede tener más de 10,000 boletos']);
        exit;
    }
} else {
...
try {
    ...
    $stmt = $pdo->prepare("INSERT INTO raffles (slug, tenant_id, title, description, organizer_name, organizer_phone, organizer_email, price_per_ticket, draw_date, total_tickets, number_start) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([$slug, current_tenant_id(), $data['title'], $description, $organizerName, $organizerPhone, $organizerEmail, $data['price_per_ticket'], $data['draw_date'], $total_tickets, $number_start]);
